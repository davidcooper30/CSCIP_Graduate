...
def run_server(host, port, **kwargs):
    ...
    if server_type == 'UDP': startServerUDP(host, port)
    elif server_type == 'RUDP': serverRUDP(host, port, f, n)
    else: startServerTCP(host, port)
...
def run_client(host, port, **kwargs):
    ...
    if port is None: port = DEFAULT_PORT
    if server_type == 'UDP': startClientUDP(host, port)
    elif server_type == 'TCP': startClientTCP(host, port)
    elif server_type == 'RUDP': clientRUDP(host, port, f, n)
...
def main():
    ...
    if args.udp: con_type = 'UDP'
    elif args.rudp: con_type = 'RUDP'
    else: con_type = 'TCP'
